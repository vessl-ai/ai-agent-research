...
class KeywordFilterToolInput(BaseModel):
    """Input schema for KeywordFilterTool."""
    urls: List[str] = Field(..., description="List of URLs to filter.")
    user_prompt: str = Field(..., description="User prompt containing keywords.")

class KeywordFilterTool(BaseTool):
    name: str = "Keyword URL Filter"
    description: str = (
        "This tool filters URLs based on keywords from the user prompt."
    )
    args_schema: Type[BaseModel] = KeywordFilterToolInput
    
    def _run(self, urls: List[str], user_prompt: str) -> str:
        # Log the exact input received
        logger.info(f"KeywordFilterTool received input - urls: {urls[:3]}... (total: {len(urls)})")
        logger.info(f"KeywordFilterTool received input - user_prompt: '{user_prompt}'")
        ...
        # Take top N most relevant URLs
        top_urls = filtered_urls[:10] if len(filtered_urls) > 10 else filtered_urls
        
        result = {
            "filtered_count": len(top_urls),
            "keywords": keywords,
